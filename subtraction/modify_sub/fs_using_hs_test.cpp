...
void FS_TEST::stimulus(){
       	srand(time(NULL));
	reset.write(1); // assert reset signal
	wait(5, SC_NS);
	cout << " reset_outside_while = " << reset.read() << endl;

	while (true){
		cout << "start" <<endl;

		//reset.write(rand() % 2);
		reset.write(0); // deassert reset signal after one clock cycle
		cout << " reset = " << reset.read() << endl;
		reset.write(rand() % 2);
		cout << " reset_randomize = " << reset.read() << endl;



		//////////////if modifying like this its not synchronizing and giving extra datas

		/*reset.write(rand() % 2); // randomly set the reset signal to 0 or 1
		cout << " reset_random = " << reset.read() << endl;
	        wait(1, SC_NS); // Wait for 1 clock cycle after setting reset signal
                reset.write(0);
		cout << " reset_after_1_cycle_delay = " << reset.read() << endl;*/
