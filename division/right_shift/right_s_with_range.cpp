...
SC_MODULE(Rshift) {
   ...
   // sc_signal<sc_uint<N> > temp_o;
	...
	o.write((0, i.read().range(N-1, shift)));     ///whit out giving hardcoded values i am using the shift_by varible for randome geeration of shift option

	 // sc_uint<S> shift = shift_by.read() ;
    //o.write((sc_biguint<N>(0), i.read().range(N-1, shift)));
